# Assimalign.Cohesion.ApiManager.ApplicationModel design

This page describes the boundaries and implementation shape of `Assimalign.Cohesion.ApiManager.ApplicationModel`.

> **Status:** Partial.

`ApiManagerResource` wraps a manifest snapshot and typed `ApiManagerResourceOptions`, and delegates to
its internal planner. `AddApiManager` returns `IApiManagerResourceDescriptor`, a thin graph-descriptor
...
- **Primary source** — `cohesion/resources/ApiManager/Assimalign.Cohesion.ApiManager.ApplicationModel/docs/DESIGN.md`.
