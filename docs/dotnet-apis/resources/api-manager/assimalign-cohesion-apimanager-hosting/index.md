...
`ApiManagerApplication.CreateBuilder(args)` returns the public concrete `ApiManagerApplicationBuilder`.
...
- **[`ApiManagerApplication`](api-manager-application.md)** — Documented public type.

`ApiManagerApplication.CreateBuilder(args)` returns the public concrete
`ApiManagerApplicationBuilder`. Explicit services preserve registration/start order and reverse
...
- **Primary source** — `cohesion/resources/ApiManager/Assimalign.Cohesion.ApiManager.Hosting/docs/OVERVIEW.md`.
