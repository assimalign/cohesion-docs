...
The `NatGatewayApplication` type belongs to `Assimalign.Cohesion.NatGateway.Hosting`.

> **Status:** Partial.

Namespace: `Assimalign.Cohesion.NatGateway.Hosting` Assembly:
`Assimalign.Cohesion.NatGateway.Hosting`

## Purpose

`NatGatewayApplication` is the public concrete application and creation entry point for the NAT
gateway hosting module. The application, builder, and context are public; runtime options remain
internal.
...
- **Primary source** — `cohesion/resources/NatGateway/Assimalign.Cohesion.NatGateway.Hosting/docs/Assembly/Assimalign.Cohesion.NatGateway.Hosting/NatGatewayApplication/OVERVIEW.md`.
