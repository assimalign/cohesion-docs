...
This page describes the boundaries and implementation shape of `Assimalign.Cohesion.LoadBalancer.Hosting`.
...
shared host lifecycle without claiming that load-balancing behavior exists.

`ProxyDataPlaneService` remains as a dormant future service stub. The filler builder does not
...
- **Primary source** — `cohesion/resources/LoadBalancer/Assimalign.Cohesion.LoadBalancer.Hosting/docs/DESIGN.md`.
