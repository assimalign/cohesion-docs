...
This page describes the boundaries and implementation shape of `Assimalign.Cohesion.EmailHub.Hosting`.
...
shared host lifecycle without claiming that email-hub behavior exists.

`MailEndpointService` remains as a dormant future service stub. The filler builder does not register
it automatically.
...
- **Primary source** — `cohesion/resources/EmailHub/Assimalign.Cohesion.EmailHub.Hosting/docs/DESIGN.md`.
