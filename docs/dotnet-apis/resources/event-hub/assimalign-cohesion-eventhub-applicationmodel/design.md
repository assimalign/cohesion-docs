# Assimalign.Cohesion.EventHub.ApplicationModel design

This page describes the boundaries and implementation shape of `Assimalign.Cohesion.EventHub.ApplicationModel`.

> **Status:** Partial.

`EventHubResource` wraps a manifest snapshot and typed `EventHubResourceOptions`, and delegates to its
internal planner. `AddEventHub` returns `IEventHubResourceDescriptor`, a thin graph-descriptor wrapper
...
`EventHubResourceControlPlane.Create` returns a fresh Hosting.Resources control plane with no accepted
...
- **Primary source** — `cohesion/resources/EventHub/Assimalign.Cohesion.EventHub.ApplicationModel/docs/DESIGN.md`.
