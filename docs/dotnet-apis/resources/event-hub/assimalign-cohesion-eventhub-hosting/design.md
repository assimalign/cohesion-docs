...
This page describes the boundaries and implementation shape of `Assimalign.Cohesion.EventHub.Hosting`.
...
shared host lifecycle without claiming that event-hub behavior exists.

`PartitionFlushService` and `IngressEndpointService` remain as dormant future service stubs. The
...
- **Primary source** — `cohesion/resources/EventHub/Assimalign.Cohesion.EventHub.Hosting/docs/DESIGN.md`.
