...
`NotificationHubApplication.CreateBuilder(args)` returns the public concrete `NotificationHubApplicationBuilder`.
...
- **[`NotificationHubApplication`](notification-hub-application.md)** — Documented public type.

`NotificationHubApplication.CreateBuilder(args)` returns the public concrete
`NotificationHubApplicationBuilder`. Explicit services preserve registration/start order and
reverse stop order. Enabled resources discover their area control plane and serve health, readiness,
...
Background-work registration belongs to the concrete `NotificationHubApplicationBuilder`:
`AddService(IHostService)` and `AddService(Func<NotificationHubApplicationContext, IHostService>)`.
...
| Reference | Kind |
|---|---|
| `Assimalign.Cohesion.NotificationHub` | `CohesionProjectReference` |
...
- **Primary source** — `cohesion/resources/NotificationHub/Assimalign.Cohesion.NotificationHub.Hosting/docs/OVERVIEW.md`.
- **Source** — `cohesion/resources/NotificationHub/Assimalign.Cohesion.NotificationHub.Hosting/src/Assimalign.Cohesion.NotificationHub.Hosting.csproj`.
