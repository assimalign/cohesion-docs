# Assimalign.Cohesion.NotificationHub.ApplicationModel design

This page describes the boundaries and implementation shape of `Assimalign.Cohesion.NotificationHub.ApplicationModel`.

> **Status:** Partial.

`NotificationHubResource` wraps a manifest snapshot and typed `NotificationHubResourceOptions`, and
delegates to its internal planner. `AddNotificationHub` returns `INotificationHubResourceDescriptor`, a
thin graph-descriptor wrapper retaining dependencies and the built plan. The planner validates the
kind, Deployment workload, declared area endpoint schemes/protocols, and http control plane at
...
- **Primary source** — `cohesion/resources/NotificationHub/Assimalign.Cohesion.NotificationHub.ApplicationModel/docs/DESIGN.md`.
