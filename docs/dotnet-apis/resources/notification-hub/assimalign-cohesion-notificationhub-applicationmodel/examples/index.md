# Examples

Examples for `Assimalign.Cohesion.NotificationHub.ApplicationModel` are derived from source documentation and code.

> **Status:** Partial.

The examples below retain real usage from project documentation, templates, fixtures, or tests. Test
examples require their original test project’s dependencies and supporting objects.

- **[Notification Hub Resource Tests](notification-hub-resource-tests.md)** — This example exercises `Assimalign.Cohesion.NotificationHub.ApplicationModel` through its co-located test source.

[Assembly overview](../index.md) · [Design](../design.md)

## Sources

- **Primary source** — `cohesion/resources/NotificationHub/Assimalign.Cohesion.NotificationHub.ApplicationModel/docs/OVERVIEW.md`.
