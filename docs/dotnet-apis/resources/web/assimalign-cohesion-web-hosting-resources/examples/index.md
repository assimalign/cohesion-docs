# Examples

Examples for `Assimalign.Cohesion.Web.Hosting.Resources` are derived from source documentation and code.

> **Status:** Partial.

The examples below retain real usage from project documentation, templates, fixtures, or tests. Test
examples require their original test project’s dependencies and supporting objects.

- **[Resource Control Plane Middleware Tests](resource-control-plane-middleware-tests.md)** — This example exercises `Assimalign.Cohesion.Web.Hosting.Resources` through its co-located test source.

[Assembly overview](../index.md) · [Design](../design.md)

## Sources

- **Primary source** — `cohesion/resources/Web/Assimalign.Cohesion.Web.Hosting.Resources/docs/OVERVIEW.md`.
