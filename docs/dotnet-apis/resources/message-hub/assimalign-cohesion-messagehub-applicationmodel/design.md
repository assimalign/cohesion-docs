# Assimalign.Cohesion.MessageHub.ApplicationModel design

This page describes the boundaries and implementation shape of `Assimalign.Cohesion.MessageHub.ApplicationModel`.

> **Status:** Partial.

`MessageHubResource` wraps a manifest snapshot and typed `MessageHubResourceOptions`, and delegates to
its internal planner. `AddMessageHub` returns `IMessageHubResourceDescriptor`, a thin graph-descriptor
...
`MessageHubResourceControlPlane.Create` returns a fresh Hosting.Resources control plane with no
...
- **Primary source** — `cohesion/resources/MessageHub/Assimalign.Cohesion.MessageHub.ApplicationModel/docs/DESIGN.md`.
