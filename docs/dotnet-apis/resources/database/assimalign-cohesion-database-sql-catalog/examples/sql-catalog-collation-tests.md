# Sql Catalog Collation Tests

This example exercises `Assimalign.Cohesion.Database.Sql.Catalog` through its co-located test source.

> **Status:** Partial.

The example reproduces
`cohesion/resources/Database/Assimalign.Cohesion.Database.Sql.Catalog/tests/SqlCatalogCollationTests.cs`
...
- **Case 1** — `Collation`: defaults and column overrides survive restart.
- **Case 2** — `Collation`: reopening a populated database under a different default is rejected.
- **Case 3** — `Collation`: legacy table formats inherit Binary.
- **Case 4** — `Collation`: non-string overrides are rejected.
