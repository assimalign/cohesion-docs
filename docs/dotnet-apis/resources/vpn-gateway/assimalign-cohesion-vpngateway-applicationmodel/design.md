# Assimalign.Cohesion.VpnGateway.ApplicationModel design

This page describes the boundaries and implementation shape of `Assimalign.Cohesion.VpnGateway.ApplicationModel`.

> **Status:** Partial.

`VpnGatewayResource` wraps a manifest snapshot and typed `VpnGatewayResourceOptions`, and delegates to
its internal planner. `AddVpnGateway` returns `IVpnGatewayResourceDescriptor`, a thin graph-descriptor
...
- **Primary source** — `cohesion/resources/VpnGateway/Assimalign.Cohesion.VpnGateway.ApplicationModel/docs/DESIGN.md`.
