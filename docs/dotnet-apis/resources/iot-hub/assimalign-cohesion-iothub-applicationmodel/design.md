# Assimalign.Cohesion.IoTHub.ApplicationModel design

This page describes the boundaries and implementation shape of `Assimalign.Cohesion.IoTHub.ApplicationModel`.

> **Status:** Partial.

`IoTHubResource` wraps a manifest snapshot and typed `IoTHubResourceOptions`, and delegates to its
internal planner. `AddIoTHub` returns `IIoTHubResourceDescriptor`, a thin graph-descriptor wrapper
...
`IoTHubResourceControlPlane.Create` returns a fresh Hosting.Resources control plane with no accepted
...
- **Primary source** — `cohesion/resources/IoTHub/Assimalign.Cohesion.IoTHub.ApplicationModel/docs/DESIGN.md`.
