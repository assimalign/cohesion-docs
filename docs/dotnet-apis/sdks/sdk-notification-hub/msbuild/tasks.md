...
The NotificationHub SDK uses shared base tasks and defines no area-specific task classes.

`Assimalign.Cohesion.Sdk.NotificationHub.Tasks` has a
`Tasks/src/Assimalign.Cohesion.Sdk.NotificationHub.Tasks.csproj` packaging project but no C# task
implementation files. No area `UsingTask` is registered. The empty
`Targets/Sdk.NotificationHub.targets` adds no task invocation.
...
[MSBuild](index.md) ·
[Packaging assembly](../assimalign-cohesion-sdk-notificationhub-tasks/index.md)
