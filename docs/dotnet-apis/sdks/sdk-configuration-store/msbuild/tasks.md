...
The ConfigurationStore SDK uses shared base tasks and defines no area-specific task classes.

`Assimalign.Cohesion.Sdk.ConfigurationStore.Tasks` has a
`Tasks/src/Assimalign.Cohesion.Sdk.ConfigurationStore.Tasks.csproj` packaging project but no C# task
implementation files. No area `UsingTask` is registered. The empty
`Targets/Sdk.ConfigurationStore.targets` adds no task invocation.
...
[MSBuild](index.md) ·
[Packaging assembly](../assimalign-cohesion-sdk-configurationstore-tasks/index.md)
