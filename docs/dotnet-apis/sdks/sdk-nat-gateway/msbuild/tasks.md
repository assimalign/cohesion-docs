...
The NatGateway SDK uses shared base tasks and defines no area-specific task classes.

`Assimalign.Cohesion.Sdk.NatGateway.Tasks` has a
`Tasks/src/Assimalign.Cohesion.Sdk.NatGateway.Tasks.csproj` packaging project but no C# task
implementation files. No area `UsingTask` is registered. The empty `Targets/Sdk.NatGateway.targets`
...
[MSBuild](index.md) · [Packaging assembly](../assimalign-cohesion-sdk-natgateway-tasks/index.md)
