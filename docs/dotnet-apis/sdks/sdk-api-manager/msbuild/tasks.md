...
The ApiManager SDK uses shared base tasks and defines no area-specific task classes.

`Assimalign.Cohesion.Sdk.ApiManager.Tasks` has a
`Tasks/src/Assimalign.Cohesion.Sdk.ApiManager.Tasks.csproj` packaging project but no C# task
implementation files. No area `UsingTask` is registered. The empty `Targets/Sdk.ApiManager.targets`
...
[MSBuild](index.md) · [Packaging assembly](../assimalign-cohesion-sdk-apimanager-tasks/index.md)
