...
The ApiManager SDK inherits shared build behavior and adds area-specific defaults.
...
The `FrameworkReference` item for `Assimalign.Cohesion.App.ApiManager` is added by `Sdk/Sdk.props`
when `CohesionAutoIncludeAppFramework` is not `false`. After substituting area names, this props
file is byte-identical to the `EmailHub`, `EventHub`, `IoTHub`, `MediaHub`, `MessageHub`,
