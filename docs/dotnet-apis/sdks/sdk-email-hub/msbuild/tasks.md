...
The EmailHub SDK uses shared base tasks and defines no area-specific task classes.

`Assimalign.Cohesion.Sdk.EmailHub.Tasks` has a
`Tasks/src/Assimalign.Cohesion.Sdk.EmailHub.Tasks.csproj` packaging project but no C# task
implementation files. No area `UsingTask` is registered. The empty `Targets/Sdk.EmailHub.targets`
...
[MSBuild](index.md) · [Packaging assembly](../assimalign-cohesion-sdk-emailhub-tasks/index.md)
