...
The IdentityHub SDK uses shared base tasks and defines no area-specific task classes.

`Assimalign.Cohesion.Sdk.IdentityHub.Tasks` has a
`Tasks/src/Assimalign.Cohesion.Sdk.IdentityHub.Tasks.csproj` packaging project but no C# task
implementation files. No area `UsingTask` is registered. The empty `Targets/Sdk.IdentityHub.targets`
...
[MSBuild](index.md) · [Packaging assembly](../assimalign-cohesion-sdk-identityhub-tasks/index.md)
