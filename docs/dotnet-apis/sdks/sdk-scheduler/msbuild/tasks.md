...
The Scheduler SDK uses shared base tasks and defines no area-specific task classes.

`Assimalign.Cohesion.Sdk.Scheduler.Tasks` has a
`Tasks/src/Assimalign.Cohesion.Sdk.Scheduler.Tasks.csproj` packaging project but no C# task
implementation files. No area `UsingTask` is registered. The empty `Targets/Sdk.Scheduler.targets`
...
[MSBuild](index.md) · [Packaging assembly](../assimalign-cohesion-sdk-scheduler-tasks/index.md)
