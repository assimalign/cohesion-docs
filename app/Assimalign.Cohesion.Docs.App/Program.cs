...
ComponentFactory components = CohesionDocsComponentCatalog.CreateFactory();
...
builder.Services.AddSingleton<ViuRouter>(services =>
    new ViuRouter(
        ...
        DocsRoutes.Create(GeneratedMarkdownContent.Catalog, new MarkdownRouteOptions
        {
    ...
    ScrollPosition? savedPosition)
{
    ...
    return savedPosition.HasValue
        ...
        : selector is not null ? new ScrollTarget(selector)
        : destination.Path != previous.Path ? new ScrollTarget(new ScrollPosition(0, 0)) : null;
}
